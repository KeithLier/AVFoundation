...
//        let urlAsset = AVURLAsset(url: URL(fileURLWithPath: path!, isDirectory: true), options: options)
    ...
    func getImageByAsset() {
        ...
        if asset.tracks(withMediaType: .video).count > 0 {
            ...
            // 异步加载多个
            imgGen.generateCGImagesAsynchronously(forTimes: [CMTime.zero as! NSValue]) { (time1, cgimage, time2, result, error) in
    ...
    // 导出视频或者音频
    func exportAsset() {
        guard let path = Bundle.main.path(forResource: "1", ofType: "pm4") else {
            return
        }
